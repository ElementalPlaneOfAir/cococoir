...
Regenerated: 2026-09-11T15:28:21Z — git fe12159
- `doc-refs` — PASS
...
Next: **T4** (leader lease on the external shared store + self-fence +
float-move). **PIVOT EXECUTED 2026-09-11 — T4 code + infra DONE:**
**PIVOT (2026-09-11, mid-T4): coordination store leaves the pair.**
Decided: drop in-pair primary/replica Redis + the promote/rejoin
state machine entirely. The store becomes ONE shared external managed
Redis; both nodes race `SET NX` on the same lease key (genuine mutual
exclusion — boot arbitration + all REPLICAOF machinery deleted; ADR-029's
"no split-brain class" claim becomes true). `lease.rs` survives 100%;
`ha.rs` shrinks ~330→~80 lines (lease free→acquire+move floats; mine→
renew; peer holds→standby; API-unreachable leader self-fences = the R4
answer). Rationale: hand-rolled Redis HA was unexpandable (2-node-only
server_id tiebreak) and was the hard-to-test part; the data plane never
touches the store (forwarder holds live listeners in memory, rehydrates
at boot — no per-packet lookups, DashMap rejected: divergent pair caches).
Provider math: Redis Cloud free fits (no command meter; ~1 op/sec vs 100
cap; heartbeat = never-inactive so 14-day-inactivity deletion can't fire);
Upstash free fails (500K cmds/mo vs ~1.8M/mo heartbeat); Hetzner managed
Redis ~€15/mo rejected.

**T4 DONE (2026-09-11, same day as pivot).** Proofs, each named:
- L0 lease: 6 live-Valkey tests in `lease.rs` green under `REDIS_URL`
  (acquire/renew/expiry/wrong-owner-renew AND release refused/resurrect).
- L0 HA reconcile: 6 tests in `ha.rs` green (takeover moves floats;
  peer-lease → standby + never-grabs-back; drift corrected while
  active; API failure → self-fence release; lease stolen mid-reconcile
  → stands down; RTO test: TTL-1s takeover + move < 15s).
- L1: new `edge-store-wiring` check PASS (template has no redis unit/
  config, no --redis-url flag, no peer flags) + full `nix flake check`
  ALL CHECKS PASSED on vermissian — including the L2 `edge-forward`
  boot test against the remodeled binary (REDIS_URL-required contract;
  test env stubs the keyed URL). Local `scripts/status.sh` 3/3 PASS.
- Teardown: edge template + `system-manager/edge.nix` — redis package,
  redis.conf, redis service, redis unit ordering, & `--redis-url` on
  ExecStart ALL deleted. `lease.rs` generics collapsed (binds concrete
  `ConnectionManager`; keep `with_key` for test isolation), peer
  flags deleted from the binary (`--node-id --server-id`, all-or-none).
- Secret wiring: `REDIS_URL` required in BOTH contracts (edge
  `crates/controlplane/secretspec.toml` + ops `secretspec.toml`
  provisioning profile/scope), `secret.rs::redis_url()` is the
  binary's default (`--redis-url` remains a dev/test override),
  provision-edge.sh writes it into edge.env. redis crate gained
  `tokio-rustls-comp` (`rediss://` TLS ready).
Residuals, honest:
1. **Store choice wired (2026-09-11):** operator entered `REDIS_URL`
   (Upstash TCP `rediss://`) into the provisioning tree of the sops
   store; verified by export through the provision scope + a live
   TLS RESP PONG. Upstash budget reality stands: free tier = 500K
   cmds/mo vs ~1.8M/mo heartbeat → it WILL exhaust; PAYG floor ≈
   $10/mo, or switch to Redis Cloud free (no meter, 100 ops/s) — a
   `REDIS_URL` change only, code is provider-agnostic.
2. One-run test flake observed (account::signup_mail_failure) then
   3 consecutive full-suite green runs; watch for recurrence, not yet a
   named bug.
3. **Store incident (2026-09-11, fixed in-session):** an unclean
   re-encrypt chain truncated the sops store; rebuilt in-session from
   the decoded values (age key untouched), stale `cococoir-edge` tree
   deleted, single `fortress-edge` tree verified. Tripwire added same
   commit: `status.sh` now resolves the full provision scope on every
   run (`provisioning-store` line) so a missing/misplaced secret fails
   ground truth immediately, never at the next provision run. Suggestion
   standing: the ENCRYPTED store file is push-safe by design — un-gitignore
   it so it is versioned (and stop carrying a single-copy secret file).

stakes unchanged: nix/nixos-modules untouched; edge.nix.tftpl edits are
template-level (L1-guarded); first real apply still is T5's hil rebuild.
Also carried: domain rename still ambiguous in-tree (proletariat.tech vs
n.tech vs interdim.net) — deprioritized, untouched by us. **T8
re-defined** (store backup/export timer on the edge, SCAN+DUMP +
client-side-encrypted rclone upload — no `BGSAVE` possible on a managed
store; lease key carries TTL so restore needs no transient-key scrub).
The old local-store durability story (AOF + `appendfsync always`) died
with the local redis; durability is the provider's concern now (accepted
in T4's risk ledger).
