...
Regenerated: 2026-09-20T03:29:59Z — git d9e871e
- `doc-refs` — PASS
...
**NEXT SESSION — T2b: the rest of the account pages (forgot/reset/
verify page/resend/verify-notice) + the machines dashboard.** T2a
landed (2026-09-20): the dioxus site now serves `/register` +
...
**Deferred**: forgot/reset/verify-page/resend/verify-notice + machines
dashboard + invites (T2b), the poem-API → axum cut (T3), wiring the
site into the deployment, and the /register /login forms still lack
their wasm client bundle for hydration (dev loop is `dx serve`). The
