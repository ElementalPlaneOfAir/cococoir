...
Regenerated: 2026-09-23T19:35:54Z — git 3909211
- `doc-refs` — PASS
...
## Works
- **comma on every fortress box; example123 demo box deleted (2026-09-23)** —
  `fortress.nix` adds `environment.systemPackages = [pkgs.comma]` (the
  nix-community `,` "run without installing" wrapper, ~3 MB unpacked) +
  `nix.settings.experimental-features` (mkDefault) so every home server has
  the operator's quick-debug tool with zero customer-facing config. The edge
  box gets it too (`edge.nix` + template) plus
  `experimental-features = nix-command flakes` in provision-edge.sh's
  nix.conf append (comma needs flakes). The tofu-rendered example123 demo
  customer box is **deleted** — the repo is a library/template for other
  people's home servers, not a shipped demo: `templates/example123.nix.tftpl`
