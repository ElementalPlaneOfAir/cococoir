...
Regenerated: 2026-08-22T15:46:05Z — git 475cd01
- `doc-refs` — PASS
...
- **Secretspec scopes for provisioning (2026-08-22)** —
  ...
  loader (`[profiles.default]`, unchanged) and the provisioning
  scripts (`[profiles.provisioning]` + `[scopes.token]` /
...
- **Static customer path half-retired (2026-08-22):** the static WG
  keypair provisioning (`gen-wg-keys.sh`, `.secrets/wg/`, the tofu WG
  locals/output, the customer wg0 peer render) is **deleted** — the
  edge owns WG identities at runtime (`GET /pubkey`). Still wired: the
  `dns.tf` per-customer wildcard and the customer wg0 *interface*
  render (no peers yet — peer wiring from `/signup` is deferred).
  Also: `redis_store_round_trip` needs a **fresh** store (asserts
  absolute `::2`/`::3` IPs) — flush before running.
...
NixOS, no disko, no nixos-anywhere. `provision-edge.sh` = secretspec
resolve (token + admin key) → `tofu apply` → install Nix →
`system-manager switch` → bring up wg0. Runtime addressing/IPs are
tofu's single source; the control plane (in `cococoir-edge`) owns
customer `/128`s + WG peers live. The edge box is live on 62.238.111.21
(was re-provisioned to Debian 2026-08-19). Secretspec-scopes arc
(2026-08-22) reworked provisioning: `[profiles.provisioning]` +
scopes, generated persisted `ADMIN_KEY` (stable hash across runs), and
WG key provisioning deleted (edge owns identities at runtime).
...
v3. DNS-on-signup landed 2026-08-21 (see Works); the static WG keypair
provisioning was retired 2026-08-22 (see secretspec-scopes arc). Next:
retire the static `dns.tf` wildcard now that runtime DNS exists, then
(ADR-025 order) auth on the control plane.
