...
Regenerated: 2026-09-18T02:10:40Z — git a58ea1d
- `doc-refs` — PASS
...
- **secrets/ single-folder provisioning config (2026-09-18,
  ...
  + `sops --decrypt`, both verified). Live tofu apply vs Hetzner not
...
**NEXT SESSION — end-user install UX for the container tier.** The
tier *works* (container-e2e + vmtest-e2e + 22 flake checks PASS,
2026-09-15). The open question is turning `nix run
.#fortress-container` (Nix + docker required, `--privileged`, host
/etc/hosts edits, `/var/lib` state resets on recreate, mac untested)
into something a non-Nix user can install. Brainstorm, don't build:
what does the "install" moment actually look like (a script? an
image on Docker Hub? a compose file? instructions?), and what is the
honest boundary — is the container tier a *product surface* or a
*developer demo*? ADR-030 has the design history; the container
tier's Works entry + landmines list the constraints.
