...
# fstab, no bootloader — nothing that can break the boot.
{
  ...
  # Unit name is `edge-control-plane`, NOT `fortress-edge`: the systemd
  # NAME PREFIX `fortress-edge` is corrupted on the edge box — systemd
  ...
  # fine under any other name (e.g. zzz-edge-a), only `fortress-edge*`
  # is refused. A non-`fortress-edge` name sidesteps it.
