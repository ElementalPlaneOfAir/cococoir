...
/// Read a cookie value from an `http::HeaderMap` (works for both the
/// poem and axum surfaces — shared by the controlplane and the site).
pub fn read_cookie_from_headers(headers: &poem::http::HeaderMap, name: &str) -> Option<String> {
    headers
        .get(header::COOKIE)?
        .to_str()
        .ok()?
...
pub(crate) fn read_cookie(req: &Request, name: &str) -> Option<String> {
    read_cookie_from_headers(req.headers(), name)
}
...
pub fn account_error_message(err: &AccountError) -> String {
    match err {
