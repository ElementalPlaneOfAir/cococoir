...
{ lib, pkgs, cfg, enabledMounts, localHost }:
let
...
in
{
  ...
  # ── Systemd mount + service pairs for each FUSE mount ───────────────────
  # Each mount becomes two units:
  #   var-lib-cococoir-mounts-<name>.mount   — the FUSE mount itself
  #   var-lib-cococoir-mounts-<name>.service — runs geesefs in foreground
  # The service holds the FUSE process; the mount unit depends on the
  # service via RequiredBy/After, the standard systemd FUSE pattern.
