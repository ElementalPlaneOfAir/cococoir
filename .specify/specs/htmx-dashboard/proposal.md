...
fortress for debugging help. The gate is local-only for now; the onward
...
An HTMX page served by `fortress-client` at `127.0.0.1:9090/` showing:
services (up/down from last probe), the last N probe spans, and the last N
journald log records per service — plus a single consent toggle ("allow fortress
...
- No new ADR. Extends ADR-008 (prober/journald/dashboard in `fortress-client`)
...
- `fortress-client` owns the dashboard. `fortress-edge` is untouched (v0
  contract frozen).
...
`nix/packages/fortress/src/health.rs`
...
**Files:** `nix/packages/fortress/src/app.rs`
...
**Files:** `nix/packages/fortress/src/otel.rs`, `nix/packages/fortress/src/lib.rs`
...
**Files:** `nix/packages/fortress/src/probe.rs`,
`nix/packages/fortress/Cargo.toml` (if a client dep is needed)
...
**Files:** `nix/packages/fortress/src/journald.rs`,
...
(e.g. `/var/lib/fortress/consent.json`); `/healthz`, `/readyz`, `/status`
unchanged. L0 via axum router tests + one manual curl in a running VM.
**Files:** `nix/packages/fortress/src/dashboard.rs`,
`nix/packages/fortress/src/consent.rs`,
`nix/packages/fortress/src/health.rs` (route wiring)
